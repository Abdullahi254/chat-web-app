...
const Page = () => {
    const [errorMessage, dispatch] = useFormState(handleRegistration, undefined)
    ...
    return (
        <main className="flex flex-col space-y-4 min-h-screen items-center py-20 px-6 max-w-4xl mx-auto">
            ...
            <form className="space-y-4 md:space-y-6 bg-white rounded-md p-4 w-[65%]" action={dispatch} >
                ...
                <div className='relative'>
                    <label htmlFor="password2" className="block mb-2 text-sm font-medium text-gray-900">Confirm </label>
                    <input type={show ? "text" : "password"} name="password2" id="password2" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required />
                    {
                        show ? <IoEyeSharp className='absolute right-3 top-10 cursor-pointer' onClick={() => setShow(prev => !prev)} /> :
                            <FaRegEyeSlash className='absolute right-3 top-10 cursor-pointer' onClick={() => setShow(prev => !prev)} />
                    }
...
function RegisterButton() {
    ...
    return (
        <button aria-disabled={pending} type="submit" className="w-full text-white bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Register</button>
    )
}
